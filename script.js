const moviesOriginalList = [
 ...
 let titleInput;
  ...
  document.getElementById('search').addEventListener('click', function(event){
   if(!movie.value){
    alert("Please Enter Movie name")
        ...
        searchResults = searchByTitle(movie.value);
    } else if(titleInput === "genre") {
        searchResults = searchByGenre(movie.value);
    }
    else{
      alert("Please select movie type!")
    }
    if(searchResults) displayResults(searchResults);
    else alert("No movies by this title or genre")
  });
  ...
  function displayResults(list) {
    ...
    if(!list){
      warning.innerHTML = `There's no movie with this title or genre `
    }
